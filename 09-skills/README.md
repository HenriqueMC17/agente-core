...
Armazenar de forma auto-contida **scripts de execução restrita e comportamentos especializados da IA**. É a central de plugins declarativos para acionamento cognitivo pontual.

## 🏗️ Papel na Arquitetura (@agente-senior-fullstack)

Camada de Comportamentos Pontuais (Tier Skill). Permite desacoplar códigos contextuais longos das diretrizes genéricas abstratas (e.g., uma habilidade isolada que analisa se as paletas de cor seguem Material Design rigoroso).

## 📂 Natureza dos Arquivos

Módulos operacionais isolados, que frequentemente possuem headers (YAML) para que o sistema de orquestração AI detecte a aplicabilidade dinâmica da skill e a convoque a tempo.
...
- **Utilitários e Bash files:** Autônomos executores para operações locais e de monitoramento sistêmico.
- **Mapeamento BeautifulMention:** Comportamentos acionados via tagging da engine local com chamadas específicas que carregam esse _context-window_.
...
- **Rigidez:** Cada pasta/habilidade deve ter seu papel singular com alta clareza de Inputs (Entrada de Dados) e Outputs (Resposta). Sem efeitos colaterais.
- **Evolução:** Mantenha os diretórios das skills limpos e atualizados para que os embeddings nativos consigam entender intuitivamente o papel de seu conteúdo ao recuperar o "Sistema".
- **Sem repetição:** Não armazene regras longas ou formativas de infra aqui. O lugar de regras é `01-global-rules`.

## 🔗 Relação com Outras Camadas

- Funciona como o arsenal (toolbox) para alimentar os processos orquestrados e estritamente definidos nos `04-workflows`.
- Tem forte dependência e ponte com `11-mcp-integrations` se exigir API de terceiros.
...
Made with [contrib.rocks](https://contrib.rocks). _(Image may be cached; [view live contributors](https://github.com/sickn33/antigravity-awesome-skills/graphs/contributors) on GitHub.)_
