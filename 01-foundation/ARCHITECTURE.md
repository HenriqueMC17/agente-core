# 01-foundation - Arquitetura de Fundação e Regras do Workspace

## 🏛️ Visão Estrutural e Arquitetural

A camada de Fundação atua como o validador sintático e semântico de nível mais baixo.
Ela organiza as regras em dois sub-módulos principais:
1. `01-global-rules`: Normas aplicáveis a todas as linguagens e frameworks do ecossistema.
2. `02-workspace-rules`: Regras aplicadas ao escopo físico do repositório local.

### 📐 Diagrama de Fluxo e Componentes Semânticos

```mermaid
graph TD
    A["Fundação e Regras do Workspace"] --> B["Políticas de Governança"]
