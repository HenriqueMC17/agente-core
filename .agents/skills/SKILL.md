...
*   **Identificador Único:** `agente-core.skills.[nome-da-categoria].[nome-do-modulo]`
*   **Camada Arquitetural:** Camada 6 (Modules)
...
| :--- | :--- | :--- |
| **Acesso a Disco** | `[Read/Write / Read-Only / None]` | Apenas caminhos dentro de `/modules/[nome-do-modulo]` |
| **Acesso a Rede** | `[Allowed / Prohibited]` | Exemplo: Apenas chamadas de API internas via localhost |
| **Variáveis de Env**| `[Required / None]` | Exemplo: Exige `DATABASE_URL` |
...
O agente de IA só poderá promover este módulo para o status `Active` se a suite de testes automáticos locais assertar os seguintes cenários:
...
- **Tratamento de Exceções (Edge Cases):**
  * Comportamento sob timeout excedido: deve rejeitar a Promise retornando código de erro `MODULE_TIMEOUT`.
