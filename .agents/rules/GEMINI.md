...
1.  **Divulgação Progressiva Obligatória:** Nunca carregue arquivos de múltiplos módulos simultaneamente na memória de contexto. Use [skills_index.json](file:///c:/Dev/agente-core/modules/skills_index.json) para selecionar e ler cirurgicamente apenas o módulo relevante.
...
2.  **Ordem de Instalação e Testes (TestSprite):** Antes de gerar ou executar casos de teste com a ferramenta do TestSprite MCP, verifique a existência do arquivo `.testsprite/config.json`. Se o arquivo existir, prossiga diretamente para a execução dos testes sem re-inicializar o bootstrap.
3.  **Visual Context (Stitch & Figma):** Ao realizar alterações visuais orientadas por design ou receber feedback de layouts do Stitch, compare a árvore de componentes estáticos antes de disparar re-gerações de telas completas.
