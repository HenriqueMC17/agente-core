...
* [/automations](file:///c:/Dev/agente-core/automations) — Scripts de automação rotineira e robôs utilitários locais.
...
* [**Catálogo de Módulos (CATALOG.md)**](file:///c:/Dev/agente-core/modules/CATALOG.md) — Índice completo renderizado em tabelas Markdown para pesquisa rápida por humanos e IAs.
* [**Índice Técnico (skills_index.json)**](file:///c:/Dev/agente-core/modules/skills_index.json) — Banco de dados JSON contendo metadados completos de cada habilidade para consumo programático por motores cognitivos.

---

## 🤝 Relação com a Central de Documentação Técnica (`Docs`)

O `agente-core` atua como a implementação prática e motor operacional de IA baseada em competências do desenvolvedor. Todas as regras sintáticas, limites constitucionais e padrões arquiteturais contidos neste repositório são herdados e alinhados diretamente com a base de conhecimento corporativo contida no [Repositório de Documentação Técnica e Governança de IA](file:///c:/Dev/Docs/README.md).

Ao atualizar regras em `/rules` ou criar novas competências em `/modules`, certifique-se de validar se há impactos ou novas tomadas de decisão que precisem ser sincronizadas com o [Relatório Completo de Análise da Pasta Docs](file:///c:/Dev/Docs/Analise_da_Pasta_Docs.md).
