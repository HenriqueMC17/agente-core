...
## 🛠️ 8. Comandos Disponíveis (Scripts Operacionais)
O diretório contém um arquivo `package.json` que expõe os seguintes scripts para validação e manutenção das habilidades:
*   `npm run validate` — Valida se as skills possuem metadados obrigatórios (name, description, category) e a seção `## When to Use`.
*   `npm run validate:strict` — Executa a validação em modo estrito.
*   `npm run index` — Compila a indexação de todas as skills no arquivo `skills_index.json`.
*   `npm run catalog` — Gera o catálogo visual e estruturado no arquivo `CATALOG.md`.
*   `npm run chain` — Executa a validação e a indexação em sequência.
*   `npm run build` — Executa todo o pipeline de sincronização (validação, indexação e catalogação).

> [!NOTE]
> Outros scripts de sincronização de repositório e visualização web (ex: `apps/web-app` e `tools/scripts/*`) foram removidos por fazerem parte do conjunto de ferramentas internas do repositório upstream, não estando presentes neste workspace local.
